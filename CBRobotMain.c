...
void initializeRobot()//initialize function
{
...
task main()
{
	RB_Button_Init(ButtonState);// initialize buttons

	initializeRobot(); // Servo etc setup
	waitForStart(); // wait for start of tele-op phase

	startTask(LiftTeleOP);// initialize Lift function
	startTask(CBDrive);// initialize drive function
	startTask(TowMechTeleOP);// initialize Tow function
	startTask(KickStandTeleOP);// initialize KickStand function
	wait1Msec(1200);
	while(true)
	{
		getJoystickSettings(joystick);//setup joystick
